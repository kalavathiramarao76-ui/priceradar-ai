...
const STORAGE_KEY = "priceradar-favorites";
...
export default function FavoriteButton({ itemId, itemLabel, size = "md", onToggle }: FavoriteButtonProps) {
  ...
  const toggle = useCallback(() => {
    ...
    // Dispatch custom event so nav can update count
  ...
  return (
    <button
      ...
      className={`group relative p-1.5 rounded-lg transition-all duration-200 hover:bg-cyan-500/10 focus:outline-none focus:ring-2 focus:ring-cyan-500/30 ${
      ...
      <Star
        className={`${sizeClasses} transition-all duration-300 ${
          isFavorite
            ...
            : "text-gray-500 group-hover:text-amber-400/60"
        }`}
      />
    </button>
  );
